...
繁中 OCR 即使通常足够快，也不能假设每一帧都必然赶在下一次点击前完成。监控开始时会
预装一个完全放行的 `WH_MOUSE_LL` 状态机；蓝色词缀指纹变化后同步武装。触发当前画面
变化的那次完整按下/抬起会正常交给游戏，从下一次按下开始，未决期间的额外按钮和滚轮
输入会被成对丢弃，不排队、不补发、不重放。未命中立即放行，命中后由居中的红色确认窗
接管，直到用户手动确认。

低级钩子用于繁中以及 POE2 English；POE1 English 保持原有路径，不安装未决闸门。停止、
异常、看门狗、关闭以及 pending 阶段的 `Ctrl + Shift + F12` 都会 fail-open；钩子无法安装
或重新武装时会拒绝开始或停止监控。该设计降低了“识别到了但手已经点到下一条”的风险，
但不宣称数学上的绝对保证。
