...
`#`, actual rolls, fixed numbers, ranges, and advanced-description forms like `8(6-8)%` all map to typed value slots; percent-vs-plain and sign are kept as structure. Everything else must match the whole line exactly — semantic neighbours like Attack/Cast, Cold/Fire, dealt/killed never cross-match. Numeric rules compare the roll the client writes in the advanced form (`20(10-20)%` — the value shown while holding Alt). Quality, catalysts, and effect-boosting crafts can make the plain tooltip show a different figure; that display scaling is not part of the compared value, and the client declares each modifier's multiplier in its `{ ... }` annotation (`— 80% Increased`). Rolled and displayed values map one-to-one per modifier, so any displayed target has an exact rolled threshold. A logical affix may span 1–4 physical lines in POE 1 and up to 8 in POE 2 (long tablet mods). One physical affix line counts at most once per match option. A hybrid — one modifier described on two lines — is written as one condition with both lines pasted together, one value slot per line; split into two conditions it can never satisfy both, because one physical modifier satisfies at most one condition.
