...
6. 识别尚未得出结果时，所有模式都不会抢走或丢弃制作点击；英文与繁中的鼠标手感一致。快速连续点击可能在几十毫秒的识别窗口内点过目标，这是快速模式为保持操作顺畅所作的明确取舍。
7. 严格命中后淡绿色浮窗消失，并在当前显示器中央弹出红色确认卡片；扫描停止。检查装备后点击“我已检查，解除鼠标锁定”，也可按 `Ctrl + Shift + F12`。确认后的约 300 ms 仍会吸收双击尾击。
...
快速模式仍直接调用 0.6.1 的单目标识别接口，旧 `settings.json` 会自动迁移且无需手工操作。多词缀规则按 POE1 / POE2 分别保存。POE1 English 使用单次严格批量 OCR，POE2 English 保留变化画面的双 Windows 引擎确认；繁中 Windows OCR 与内置 Paddle 路径均支持共享的多目标局部复核。每次扫描的额外复核次数有固定上限，目标数量不会导致重复整帧 OCR；普通识别与辅助识别共享物理行身份，不能把同一词缀重复计数。识别模板与阈值保持不变，快速模式不再在命中前拦截鼠标。
...
- 本机多次 Release 回归中，原生大选区的 changed-frame 预检 p50 `3.0–3.1 ms`、p95 `3.3–4.8 ms`；目标判定 p50 `27.0–29.2 ms`、p95 `53.0–55.9 ms`。27 个目标中 23 个直接命中，4 个使用局部复核；首次懒加载内置兼容模型的单次最坏判定为 `181–229.2 ms`。这段识别时间内鼠标仍完全直通。
...
- 红色告警包含全屏粗边框、显示器中央确认卡、立即可用的手动确认按钮和循环 WAV；确认后的 300 ms 继续吸收双击尾部。
- 英文、繁中和 POE2 在识别未决期间都保持鼠标完全直通，不安装或武装命中前的低级鼠标闸门，因此不会打断装备悬停，也不会造成点击丢失。只有严格命中并验证红色告警窗口确实可见、可点击后，窗口才开始阻挡后续输入。
...
程序只读取屏幕像素，绝不向游戏合成、补发、重放或排队输入。快速模式在确认命中前不会安装或武装系统低级鼠标闸门，所有按键和滚轮都直接交给游戏；这保证了操作手感，但也意味着玩家在 OCR 完成前继续点击时仍可能点过目标。

严格命中后，程序先显示并验证独立的红色全桌面告警窗，再由该窗口阻挡后续输入。手动确认、停止、异常或关闭都会恢复放行；确认动作还会等待约 300 ms 且所有鼠标键抬起，避免确认双击的尾部落进游戏。若红窗无法可靠显示，程序会明确报错并停止本次监控，不把隐藏窗口当成保护。
